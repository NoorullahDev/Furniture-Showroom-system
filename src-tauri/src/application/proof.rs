...
pub async fn app_info(state: &AppState) -> Result<AppInfoDto, AppError> {
    ...
    let latest = infrastructure::db::MIGRATOR
        .iter()
        .map(|m| m.version)
        .max()
        .unwrap_or(0);
...
pub async fn import_image(
    state: &AppState,
    path: String,
) -> Result<ImageImportResultDto, AppError> {
...
pub async fn create_backup(state: &AppState) -> Result<BackupResultDto, AppError> {
    ...
    let backup =
        tokio::task::spawn_blocking(move || infrastructure::create_backup(&db_path, &backups_dir))
            .await
            .map_err(|e| AppError::Internal(format!("background task failed: {e}")))??;
...
pub async fn open_path(state: &AppState, path: String) -> Result<(), AppError> {
    ...
    opener::open(&target).map_err(|e| AppError::Internal(format!("failed to open file: {e}")))
}
